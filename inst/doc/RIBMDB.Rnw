...
%\VignetteDepends{RIBMDB}
%\VignetteKeywords{databases}
%\VignetteKeywords{RIBMDB}
%\VignettePackage{RIBMDB}
...
\noindent
Package \pkg{RIBMDB} implements ODBC database connectivity.  It was
...
Ripley.  Version 1.0-1 was released in January 2003. This was later 
used to develop \pkg{RIBMDB} which is nowadays a mature and much-used 
platform for interfacing R to DB2 systems.
...
\pkg{RIBMDB} works with ODBC version 3.  This number describes both the
...
capable than others: in the main \pkg{RIBMDB} works with basic
...
Two groups of functions are provided in \pkg{RIBMDB}.  The mainly
...
the client (here \pkg{RIBMDB}) to the DBMSs as specified in the DSN.
Such connections are normally used throughout a session, but should be
closed explicitly at the end of the session---however \pkg{RIBMDB} will
...
\pkg{RIBMDB} functions.  If you forget the details, printing it will
...
The simplest and most common use of \pkg{RIBMDB} is to extract data
...
underscore, and \pkg{RIBMDB} does not in general support vendor
...
quoting rules for names \pkg{RIBMDB} itself sends, but sensible
...
are stored as date-times).  The files in the \code{RIBMDB/tests}
...
change rows in an existing table.  Note that \pkg{RIBMDB} only does
...
\pkg{RIBMDB} allows primary keys to be set as part of the
...
\item[Character types] Character types can be classified three ways:
  ...
  Note that currently \pkg{RIBMDB} only uses the current locale for
...
\item[Dates and times] The handling of dates and times is very much
  ...
  All such types are transferred as character strings in \pkg{RIBMDB}.

\item[Binary types]
  These are less common, and unsupported by \pkg{RIBMDB} prior to
...
\pkg{RIBMDB}) when transferring data and are the same for all drivers.
...
The only ODBC C data types currently used by \pkg{RIBMDB} are
...
\pkg{RIBMDB} usually works with tables in the current schema, but unless
...
supported by the \pkg{RIBMDB} functions.
...
client-server system, and the ODBC client (\pkg{RIBMDB}) and the server
...
this is set when \pkg{RIBMDB} is loaded from the setting of the current
...
65535, the default \pkg{RIBMDB} limit on transfers).
...
\pkg{RIBMDB} is simple to install, and binary distributions are
...
something like
\begin{example}
ODBC_INCLUDE=/path/to/libiodbc-srcs/include R CMD INSTALL RIBMDB
\end{example}
\noindent
...
RIBMDB connection objects are an integer with several attributes: they
...
handle for the environment that \code{RIBMDB} opens when a connection
...
Prior to \pkg{RIBMDB} 1.3-0 the default was to fetch a row at a time,
