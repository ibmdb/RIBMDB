...
static void odbcInit(void)
{
	...
	if(retval == SQL_SUCCESS || retval == SQL_SUCCESS_WITH_INFO) {
	    ...
	    error(_("[RIBMDB] ERROR: Could not SQLAllocEnv"));
    }
}
...
SEXP RODBCDriverConnect(SEXP connection, SEXP id, SEXP useNRows, SEXP ReadOnly)
{
	...
	warning(_("[RIBMDB] ERROR:invalid connection argument"));
	UNPROTECT(1);
	return ans;
    }
	...
	if(retval == SQL_SUCCESS || retval == SQL_SUCCESS_WITH_INFO) {
		...
		while(1) {
		    ...
		    warning(_("[RIBMDB] ERROR: state %s, code %d, message %s"),
			    state, code, msg);
		}
	    } else warning(_("[RIBMDB] ERROR: Could not SQLDriverConnect"));
	    (void)SQLFreeHandle(SQL_HANDLE_DBC, thisHandle->hDbc);
	}
    } else {
	warning(_("[RIBMDB] ERROR: Could not SQLAllocConnect"));
    }
    UNPROTECT(1);
    return ans;
}
...
SEXP RODBCQuery(SEXP chan, SEXP query, SEXP sRows)
{
	...
	if( res != SQL_SUCCESS && res != SQL_SUCCESS_WITH_INFO ) {
	    ...
	    sprintf(message,
		    "[RIBMDB] ERROR: Could not SQLExecDirect '%s'", cquery);
	    geterr(thisHandle);
...
SEXP RODBCPrimaryKeys(SEXP chan, SEXP table, SEXP cat, SEXP schem)
{
	...
	res = SQLPrimaryKeys( thisHandle->hStmt,
			      ...
			      SQL_NTS);
	success_or_failure(_("[RIBMDB] ERROR: Failure in SQLPrimary keys"));
    }
    return ScalarInteger(stat);
}
...
SEXP RODBCColumns(SEXP chan, SEXP table, SEXP cat, SEXP schem, SEXP sLiteral)
{
	...
	res = SQLColumns( thisHandle->hStmt,
			  ...
			  (SQLCHAR *) translateChar(STRING_ELT(table, 0)),
			  SQL_NTS, NULL, 0);
	success_or_failure(_("[RIBMDB] ERROR: Failure in SQLColumns"));
    }
    return ScalarInteger(stat);
}
...
SEXP RODBCSpecialColumns(SEXP chan, SEXP table, SEXP cat, SEXP schem)
{
	...
	success_or_failure(_("[RIBMDB] ERROR: Failure in SQLSpecialColumns"));
    }
    return ScalarInteger(stat);
}
		 ...
		 SEXP sLiteral)
{
	...
	res = SQLTables(thisHandle->hStmt,
			...
			(SQLCHAR *) tName, len3,
			(SQLCHAR *) tType, len4);
	success_or_failure(_("[RIBMDB] ERROR: SQLTables failed"));
    }
    return ScalarInteger(stat);
}
...
static int cachenbind(pRODBCHandle thisHandle, int nRows)
{
    ...
    if( retval != SQL_SUCCESS && retval != SQL_SUCCESS_WITH_INFO ) {
	geterr(thisHandle);
	errlistAppend(thisHandle, _("[RIBMDB] ERROR: SQLRowCount failed"));
	goto error;
    }
	...
	if( retval != SQL_SUCCESS && retval != SQL_SUCCESS_WITH_INFO ) {
	    ...
	    errlistAppend(thisHandle, 
			  _("[RIBMDB] ERROR: SQLDescribeCol failed"));
	    goto error;
	}
	...
	if( retval != SQL_SUCCESS && retval != SQL_SUCCESS_WITH_INFO ) {
	    geterr(thisHandle);
	    errlistAppend(thisHandle, _("[RIBMDB] ERROR: SQLBindCol failed"));
	    goto error;
	}
    }
    return 1;
...
SEXP RODBCNumCols(SEXP chan)
{
    ...
    if(NCOLS == -1)
	errlistAppend(thisHandle, _("[RIBMDB] No results available"));
		 ...
		 SEXP params, SEXP test)
{
	...
	char *message = Calloc(strlen(cquery)+50, char);
	sprintf(message,
		"[RIBMDB] ERROR: Could not SQLPrepare '%s'", cquery);
	geterr(thisHandle);
	errlistAppend(thisHandle, message);
	(void)SQLFreeHandle(SQL_HANDLE_STMT, thisHandle->hStmt);
	thisHandle->hStmt = NULL;
	...
	if(res  != SQL_SUCCESS && res != SQL_SUCCESS_WITH_INFO) {
	    geterr(thisHandle);
	    errlistAppend(thisHandle, _("[RIBMDB] SQLBindParameter failed"));
	    geterr(thisHandle);
	    stat = -1;
	    goto end;
	}
	...
	if(vtest < 2) {
	    ...
	    if( res != SQL_SUCCESS && res != SQL_SUCCESS_WITH_INFO ) {
		errlistAppend(thisHandle, _("[RIBMDB] Failed exec in Update"));
		geterr(thisHandle);
		stat = -1;
		goto end;
	    }
...
static int inRODBCClose(pRODBCHandle thisHandle)
{
	...
	warning(_("[RIBMDB] Error in SQLDisconnect"));
	success = -1;
    }
    retval = SQLFreeHandle(SQL_HANDLE_DBC, thisHandle->hDbc);
    if( retval != SQL_SUCCESS && retval != SQL_SUCCESS_WITH_INFO ) {
	warning(_("[RIBMDB] Error in SQLFreeconnect"));
	success = -1;
    }
    cachenbind_free(thisHandle);
    errorFree(thisHandle->msglist);
...
/* do this strdup so that all the message chain can be freed*/
	...
	REprintf("RIBMDB: Memory Allocation failure for message string\n");
	return;
    }
...
void R_init_RIBMDB(DllInfo *dll)
{
